...
exports.getAnalyticsOverview = asyncHandler(async (req, res, next) => {
  ...
  const analytics = {
    ...
    // Consistency data (premium feature)
    consistency:
      req.user.subscriptionStatus === "premium"
        ? calculateConsistencyData(checkins, habits)
        : null,
  };
...
// @access  Private
exports.getHabitAnalytics = asyncHandler(async (req, res, next) => {
  ...
  const checkins = await Checkin.find({ habitId: req.params.id });
  ...
  const analytics = {
    ...
    // Streak history (premium feature)
    streakHistory:
      req.user.subscriptionStatus === "premium"
        ? calculateStreakHistory(checkins)
        : null,

    // Advanced patterns (premium feature)
    patterns:
      req.user.subscriptionStatus === "premium"
        ? {
...
// @desc    Get export of user data (premium feature)
...
// Helper function to calculate completion for a period
...
// Helper function for consistency data
